...
void wifi_config(WifiConfig* config) {
    strncpy(wifiConfig.ssid, config->ssid, (size_t)WIFI_SSID_SIZE_LIMIT);
    strncpy(wifiConfig.password, config->password, (size_t)WIFI_PASSWORD_SIZE_LIMIT);
...
esp_err_t wifi_connect(void) {
    if (!isWifiConfigured) {
        ESP_LOGE(TAG, "Wifi is not configured. You must call wifi_config()");
...
static esp_netif_t* wifi_start() {
    ...
    wifi_config_copy(&wifi_config, &wifiConfig);
...
static void wifi_stop() {
    esp_netif_t* wifi_netif = wifi_get_netif_from_desc("sta");
