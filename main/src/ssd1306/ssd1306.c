...
esp_err_t ssd1306_init(SSD1306_t * dev, int width, int height)
{
	if (dev->_address == SPIAddress) {
		return spi_init(dev, width, height);
	} else {
		return i2c_init(dev, width, height);
	}
}

void ssd1306_display_text_with_offset(SSD1306_t * dev, int page, const char * text, int text_len, bool invert, uint8_t offsetX)
{
...
{
    ssd1306_display_text_with_offset(dev, page, text, text_len, invert, 0);
}
